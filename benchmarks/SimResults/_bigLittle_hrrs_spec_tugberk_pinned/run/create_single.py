...
./sniper/run-sniper -s mytrace:stats.out -n 4 -c gainestown -c scheduler/big,scheduler/small,scheduler/small,scheduler/small -g --scheduler/type=pinned -g --scheduler/pinned/quantum=1000000 -s demorr_pinned.py --sim-end=last-restart --power -d ~/data.$JOB_ID/result --traces=%(a)s.0.sift,%(a)s.0.sift,%(a)s.0.sift,%(a)s.0.sift
...
for [CSCRATCHS, CSCRATCH, DATA, v] in [['/scratch/nas/1/dn/sniper-6.0', '/scratch/nas/1/dn/sniper-6.0/benchmarks', 'data.$JOB_ID', '_bigLittle_hrrs_spec_tugberk_pinned']]:
  for i in cpu_apps:
    del min_cpu_apps[0]
