...
./sniper/run-sniper -s mytrace:stats.out -n 4 -c gainestown -c scheduler/big,scheduler/small,scheduler/small,scheduler/small -g --scheduler/type=pinned -g --scheduler/pinned/quantum=4000000 -s demorr_pinned.py --sim-end=first --power -d ~/data.$JOB_ID/result --traces=%(a)s.0.sift,%(label)s.0.sift,%(b)s.0.sift,%(c)s.0.sift
...
torun = '#!/bin/sh\n'
#hd = 'multi_'
apps = [['gcc','bzip2', 'perlbench', 'bwaves'], ['mcf','gamess', 'milc', 'zeusmp'], [ 'namd', 'cactusADM','gromacs', 'leslie3d'], [ 'soplex', 'gobmk','povray', 'calculix'], [ 'sjeng','hmmer', 'GemsFDTD', 'libquantum'], [ 'omnetpp','h264ref', 'lbm', 'tonto'], [ 'xalancbmk','astar', 'leslie3d', 'namd']]



for [CSCRATCHS, CSCRATCH, DATA, v] in [['/scratch/nas/1/dn/sniper-6.0', '/scratch/nas/1/dn/sniper-6.0/benchmarks', 'data.$JOB_ID', 'Paper2_pinned_spec_base']]:
