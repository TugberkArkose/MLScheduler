...
./sniper/run-sniper -s mytrace:stats.out -n 4 -c gainestown -c scheduler/big,scheduler/small,scheduler/small,scheduler/small -g --scheduler/type=pinned -g --scheduler/pinned/quantum=1000000 -s demorr_pinned.py --sim-end=last-restart --power -d ~/data.$JOB_ID/result --traces=%(a)s.0.sift,%(label)s.0.sift,%(b)s.0.sift,%(c)s.0.sift
...
for [CSCRATCHS, CSCRATCH, DATA, v] in [['/scratch/nas/1/dn/sniper-6.0', '/scratch/nas/1/dn/sniper-6.0/benchmarks', 'data.$JOB_ID', 'micro_pinned_train_combos']]:
  for i in cpu_apps:
    ...
    for j in cpu_apps:
      ...
      for k in cpu_apps:
        ...
        for t in cpu_apps:
          ...
          for [ext, hd, sch, script,label] in [['1','cmpA_','scheduler/pinned', '',j]]:
            ...
            f.close()
  for [ext, hd, sch, script] in [['1','cmpA_','scheduler/pinned', '']]:
    for i in apps:
