...
./sniper/run-sniper -s mytrace:stats.out -n 4 -c gainestown -c scheduler/big,scheduler/small,scheduler/small,scheduler/small -g --scheduler/type=pinned -g --scheduler/pinned/quantum=1000000 -s demorr_pinned.py --sim-end=last-restart --power -d ~/data.$JOB_ID/result --traces=%(c)s.0.sift,%(a)s.0.sift,%(label)s.0.sift,%(b)s.0.sift
    ...
    for j in cpu_apps:
      ...
      for k in cpu_apps:
        ...
        for t in cpu_apps:
          ...
          for [ext, hd, sch, script,label] in [['1','cmpB_','scheduler/pinned', '',j]]:
	    ...
	    #for item in torun:
            ...
            f.close()
  for [ext, hd, sch, script] in [['1','cmpB_','scheduler/pinned', '']]:
    for i in apps:
