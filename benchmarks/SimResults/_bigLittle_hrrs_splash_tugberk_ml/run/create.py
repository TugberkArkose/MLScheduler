...
./sniper/run-sniper -s mytrace:stats.out -s MLScheduler.py -g --scheduler/type=pinned -g --scheduler/pinned/quantum=1000000 --benchmarks=splash2-%(a)s-large-%(ext)s,splash2-%(label)s-large-%(ext)s,splash2-%(a)s-large-%(ext)s,splash2-%(label)s-large-%(ext)s -n 4 -c gainestown -c scheduler/big,scheduler/small,scheduler/small,scheduler/small --sim-end=last-restart --power -d ~/data.$JOB_ID/result 
...
for [CSCRATCHS, CSCRATCH, DATA, v] in [['/scratch/nas/1/dn/sniper-6.0', '/scratch/nas/1/dn/sniper-6.0/benchmarks', 'data.$JOB_ID', '_bigLittle_hrrs_splash_tugberk_ml']]:
  for i in splash2_apps:
    del min_splash_apps[0]
    for j in min_splash_apps: 
      for [ext, sch, script, hd, label] in [['1','scheduler/hrrs', '', 'Combo_',j]]:
