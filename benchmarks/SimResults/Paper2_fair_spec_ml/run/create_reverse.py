...
for [CSCRATCHS, CSCRATCH, DATA, v] in [['/scratch/nas/1/dn/sniper-6.0', '/scratch/nas/1/dn/sniper-6.0/benchmarks', 'data.$JOB_ID', 'Paper2_fair_spec_ml']]:
  for [ext, hd, sch, script] in [['1','cmp_','scheduler/pinned', '']]:
    for i in apps:
      ...
      array += '%(label)s%(a)s%(b)s%(c)s_reverse '%{'label':i[0], 'a':i[1], 'b':i[2], 'c':i[3]}
      f = open('torun_cpu_all_combination_reverse.sh', 'w')
      f.write(torun)
      f.close()
