...
class SchedulerLocality:
    ...
    MODELNAME = "/scratch/nas/3/dn/sniper-6.0/scripts/round_robin_model.p"

    def setup(self, args):
        args = args.split(':')
        if sys.argv[0]:
            self.MODELNAME = str(sys.argv[0])
            print (self.MODELNAME)
        if sys.argv[1]:
            self.reverse = True
        if self.reverse:
            self.PATH = "python /scratch/nas/3/dn/sniper-6.0/scripts/predictor_blackbox_reverse.py "
        print "setup"
    ...
    def predict(self, a, b, c, d):
        ...
        proc = os.popen(self.PATH + str(1) + " " + self.MODELNAME + " " + a + " " + b +
                        " " + c + " " + d).read()
        #result = json.loads(proc)
        #code above does not work check why
        result = proc
    ...
    def periodic(self, time, time_delta):
        ...
        if self.hetero_timer == 4:
            # Update fair scheduler scores
            [thread.updateHeteroScore() for thread in self.threads.values()]
            self.hetero_timer = 0
        self.hetero_timer += 1
        ...
        for thread in threads:
            print "score " + str(thread.hetero_score) + " ID " + str(
                thread.thread_id)
        if len(threads) >= 4:
    ...
    def train(self, train_data):
        ...
        proc = os.popen(self.PATH + str(0) + " " + self.MODELNAME + " " + jlist + " " +
                        statList + " ").read()
        #result = json.loads(proc)
        #code above does not work check why
        result = proc
