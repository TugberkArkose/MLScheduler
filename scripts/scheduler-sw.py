...
class Thread:
  ...
  def __init__(self, thread_id, getScoreMetric, mask):
  ...
  def setCore(self, core_id, time = -1):
    ...
    if core_id is None:
      ...
      sim.thread.set_thread_affinity(self.thread_id, [ c in self.mask for c in range(sim.config.ncores) ])
...
class SchedulerLocality:
  ...
  def setup(self, args):
    ...
    self.hwthreads = long(args.get(3, None) or 4)
    ...
    if core_mask:
      ...
      self.mask = [ core for core in range(sim.config.ncores) if core_mask[core] ]
    else:
      self.mask = range(sim.config.ncores)
  ...
  def hook_thread_start(self, thread_id, time):
    self.threads[thread_id] = Thread(thread_id, self.getScoreMetric, self.mask)
