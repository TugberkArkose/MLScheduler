...
class Scheduler:
    ...
    stats = {}
    path = "python /scratch/nas/1/dn/sniper-6.0/benchmarks/SimResults/myNumpy.py "
    ...
    def swapThreads(self, combination):
        ...
        sim.thread.set_thread_affinity(combination[0], ([core == 0 for core in range(sim.config.ncores)]))
        sim.thread.set_thread_affinity(combination[1], ([core == 1 or core == 2 or core == 3 for core in range(sim.config.ncores)]))
        sim.thread.set_thread_affinity(combination[2], ([core == 1 or core == 2 or core == 3 for core in range(sim.config.ncores)]))
        sim.thread.set_thread_affinity(combination[3], ([core == 1 or core == 2 or core == 3 for core in range(sim.config.ncores)]))
    ...
    def print_info(self):
        ...
        print 'Total ipc: ', (self.ipc[0] + self.ipc[1] + self.ipc[2] + self.ipc[3])
