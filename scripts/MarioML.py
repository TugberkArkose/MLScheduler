...
class Thread:
    ...
    def __repr__(self):
        return 'Thread(%d,ipc = %f, %s, score = %f)' % (self.thread_id, self.BigIpc if self.core in BIG  else self.SmallIpc, 'core = %d' % self.core if self.core is not None else 'no core', self.score)
...
class SchedulerLocality:
    ...
    def periodic(self, time, time_delta):
        ...
        for thread in threads:
            big_queue[thread.thread_id] = thread.BigIpc
            small_queue[thread.thread_id] = thread.SmallIpc

        combination = []

        combination.append(sorted(big_queue, key=big_queue.get, reverse=True)[:len(BIG)])
        print combination
        combination = [item for sublist in combination for item in sublist]

        for key in small_queue.keys():
            if key in combination:
                del small_queue[key]

        print small_queue

        small_combination = (sorted(small_queue, key=small_queue.get, reverse=True)[:len(SMALL)])
        print small_combination

        combination += small_combination

        print combination

        new_thread = []
        for thread_id in combination:
            for thread in threads:
                if thread.thread_id == thread_id:
                    new_thread.append(thread)
    ...
    def printPredectedIPC(self):
        ...
        for thread in self.threads.values():
            if thread.core in BIG:
                print "Big ", thread.BigIpc, " Small* ", thread.SmallIpc
